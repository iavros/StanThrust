...
\title{\textbf{StanThrust Solver Notes}\\\large Geometry, Feed-System, MOC, Navier-Stokes, Thermal, and Structural Formulation}
...
The repository keeps shared inputs and output contracts centralized while preserving separate files for physically distinct numerical models. This avoids duplicated constants without combining unrelated equations into one oversized module. The implementation map below links each active runtime path to its source files.
...
These are not reconstructed inside the geometry model; they are taken from the Cantera-backed thermochemistry provider and inserted directly into the coupled chamber/nozzle station model. Cantera is a required dependency for supported solver runs, and missing thermochemistry is reported as a solver error.
