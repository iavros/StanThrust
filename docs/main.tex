...
\section{Purpose}
StanThrust is a liquid-engine sizing and analysis tool with a coupled design solver stack. The software takes a mission request, converts it into a geometry bundle, enforces an architecture-aware feed-pressure state, evaluates a burn-time feed transient, calls a Cantera thermochemistry provider, and then evaluates thrust, impulse, structural margins, and export geometry with a chamber/nozzle flow model. The software exposes fast preview, refined characteristic-net, and Navier-Stokes design modes for explicit solves. The final desktop solve always escalates to the Navier-Stokes design path so the saved design state is based on the most detailed in-app simulation.
...
At every time step, the total propellant flow supplied by the design or coupled solver is split by mixture ratio:
\begin{equation}
\dot{m}_p=\dot{m}_{p,\text{solver}},
\qquad
...
\end{equation}
and the corresponding nondimensional pump-speed index is
\begin{equation}
N_k^*=\clip\!\left(\frac{P_{\text{pump},k}}{P_{\text{pump},0}},\,0.82,\,1.18\right).
\label{eq:pump-speed-index}
\end{equation}
...
\item \textbf{Navier-Stokes mode}, which starts from the Cantera and MOC station field, applies shock feedback, and then applies the in-app axisymmetric viscous finite-volume correction.
\end{enumerate}
The final solve performed by the desktop app forces Navier-Stokes mode with a dense axial station count. Lower modes remain useful for comparison plots and quick inspection, but they are not the final design basis.
...
\begin{axis}[
  ...
  ybar,
  bar width=10pt,
  symbolic x coords={fast,refined,navier_stokes},
  xtick=data,
  xticklabels={Fast,Refined,Navier-Stokes},
  ylabel={Thrust [N]},
  grid=major,
]
...
\end{tikzpicture}
\end{minipage}
\caption{Flow-mode comparison for the packaged sample case. The final app solve uses the Navier-Stokes path.}
\end{figure}

\subsection{Final-Pass Uncertainty Bounds}
The final coupled result includes local uncertainty bounds for the headline outputs. These are not optimizer weights and they are not calibration factors. They are solver diagnostics derived from the final Navier-Stokes pass:
\begin{itemize}[leftmargin=5mm]
\item coupled pressure residual;
\item final thrust residual;
\item shock-feedback state;
\item final Navier-Stokes momentum residual;
\item heat-transfer and material-margin state.
\end{itemize}
For a solved value $y$, StanThrust reports
\begin{equation}
y_{\mathrm{low}}=y\left(1-\frac{u_-}{100}\right),
\qquad
y_{\mathrm{high}}=y\left(1+\frac{u_+}{100}\right),
\label{eq:uncertainty-bounds}
\end{equation}
where $u_-$ and $u_+$ are field-specific percentages. The pressure, thrust, mass-flow, specific-impulse, wall-temperature, and combined material-margin rows are exported in the coupled solver payload. The bounds are intended for design review and regression comparison. External CFD and hot-fire data are still required before treating the numbers as hardware-validation values.
...
\end{equation}
where $\mu_g$ is calculated with a Sutherland-law temperature correction. The gas Nusselt number is
\begin{equation}
\mathrm{Nu}_g=
\begin{cases}
...
The most useful benchmark is not a simple side-by-side list of thrust levels. A better engineering check is to reconstruct a small set of public student engines, run those reconstructed cases through the same StanThrust solver used for the final design pass, and then compare the solver output against the published operating point. The benchmark code does not run an optimizer and does not tune inputs to match the references. Each case is a direct Navier-Stokes solver call from the public operating-point data and the stated reconstruction assumptions.
...
\item thermal quantities are bounded engineering calculations;
