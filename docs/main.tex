...
The desktop 3D views are rendered from the same solved scalar fields and sampled nozzle contour used by the measurement table and exports. The drawing layer may choose colors, perspective, wireframe density, and label positions, but the visible engine dimensions are not hard-coded in the renderer. For example, tank cylinders use solved outer diameters and required lengths, pump models use solved casing, impeller, eye, hub, blade-count, and motor-envelope values, and the impinging injector pattern uses the solved element count, orifice diameter, pair spacing, ring diameter, and convergence height.

Cooling geometry is treated the same way. Regenerative ribs use the solved channel count, rib height, rib thickness, and channel depth. Film cooling uses the solved injector-face diameter, slot height, slot width, slot count, and injection angle. In the exterior 3D view, film cooling is shown as inlet-face slot hardware and short entry ticks only. The coolant film itself is an internal boundary-layer flow, not an exterior solid feature, so the renderer does not draw long film lines down the chamber body. This keeps the render faithful to the calculated hardware while avoiding a misleading solid-geometry interpretation of an internal flow layer.
...
These are not reconstructed inside the geometry model; they are taken from the Cantera-backed thermochemistry provider and inserted directly into the quasi-one-dimensional performance model. Cantera is a required dependency for supported solver runs, and missing thermochemistry is reported as a solver error.
...
The combustion solve now calls a reduced-order heat-transfer model after the quasi-one-dimensional flow state has been assembled. The model evaluates three representative wall sections: chamber midsection, throat region, and nozzle exit plane. Each section uses the local diameter, length, Mach number, material, wall thickness, and cooling configuration from the solved design state.

For a station with Mach number $M$, the gas-side static state is
\begin{equation}
...
\end{equation}
where $\mu_g$ is estimated with a Sutherland-law temperature correction. The gas Nusselt number is
\begin{equation}
\mathrm{Nu}_g=
\begin{cases}
3.66+0.008\sqrt{\Rey_g}, & \Rey_g<2300\\
0.026\Rey_g^{0.8}\Pr_g^{0.4}, & \Rey_g\ge 2300
\end{cases}
...
\begin{equation}
T_{r,\mathrm{eff}}=T_r-\eta_{\mathrm{film}}\left(T_r-T_{\mathrm{ref}}\right).
...
Each wall section is then solved as a gas-wall-coolant resistance network:
\begin{equation}
...
\end{equation}
The section heat load and coolant outlet rise are
\begin{equation}
\dot{Q}=q''\pi dL,
\qquad
...
\end{equation}
For passive sections, the outer sink is ambient convection. For regenerative sections, the coolant outlet from each solved section becomes the inlet bulk temperature for the next section.
...
\end{equation}
and a temperature-derated allowable stress
\begin{equation}
\sigma_{\mathrm{allow},T}=\phi_T(T_{\mathrm{hot}},T_{\mathrm{limit}})\sigma_{\mathrm{allow}}.
\label{eq:material-derated-stress}
\end{equation}
The derating factor remains unity below $0.55T_{\mathrm{limit}}$, falls linearly to $0.45$ at the material temperature limit, and continues decreasing if the predicted hot wall exceeds the limit.
...
\begin{enumerate}[leftmargin=6mm]
\item regenerate this report's CSV assets with \texttt{docs/overleaf\_solver\_report/generate\_assets.py};
